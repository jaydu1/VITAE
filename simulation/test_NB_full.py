...
## The simulated data is generated with all high variable genes
all_hgv_data = ["linear","linear_1","linear_2","linear_3",
                "bifurcation","bifurcating_1","bifurcating_2","bifurcating_3",
...
model = VITAE.VITAE(adata = dd,
    #        adata_layer_counts = 'counts',
            hidden_layers = [32, 16],
            latent_space_dim = 16,
            model_type = data['type'],adata_layer_counts="count")
...
PATH = 'result_NB/%s/%s/'%(file_name,'weight')
...
if os.path.exists(os.path.join(path,'train.checkpoint.index')):
    ...
    try:
        model.init_latent_space(cluster_label='grouping')
    except:
    ...
    model.train(beta=1, learning_rate=1e-3, early_stopping_tolerance=0.01,
                early_stopping_relative=True, verbose=False,
